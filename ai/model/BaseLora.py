...
class BaseLoraModel(BaseModel):
    ...
    def attach_lora(self, adapter_name:str = "person"):
        local_path = self.model_info.model_path[:-(len(self.model_info.lora_filename) + 1)]
        self.pipe.load_lora_weights(local_path, weight_name=self.model_info.lora_filename + ".safetensors", adapter_name=adapter_name)
